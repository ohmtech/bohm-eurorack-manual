...
then represents the total duration of the kick, and {guilabel}`VELOCITY` its
volume.

{guilabel}`HIT` can work both as a trigger or a gate control. If {guilabel}`HIT`
remains on (for example by keeping the button pressed), the kick will produce
sound. When {guilabel}`HIT` is released, the sound will decay.
...
The wavetable and transient synthesizers are then mixed and enter the effect
...
reverb, noise and gritty noise. The sound generators can be selected and blended
...
as well as the {guilabel}`TAPS` CV input, is available on the {guilabel}`TAPS` CV output.
...
but the actual activation or deactivation happens synchronously to the {guilabel}`HIT`
trigger. The {guilabel}`FX` controls the effect parameter.
...
When `FX` is set to `DJ FILTER`, the following controls the resonance of the DJ filter:

- `NEUTRAL` sets the Performer DJ Filter resonance to none
- `RAVE` sets the Performer DJ Filter resonance to have a more pronounced "rave" effect
