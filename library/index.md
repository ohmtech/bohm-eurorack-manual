...
The FM-2X model is a 2-operator FM kick.
The second operator is a sub-bass operator
...
The second operator itself is a wavetable oscillator, and {guilabel}`COLOR`
...
The first operator modulates the frequency of the second to bring the transient
...
The first operator is itself as well a wavetable oscillator, and {guilabel}`TRS TONE`
...
The two operators frequencies are bound by a ratio, which can be adjusted
in the model variations menus using the `RATIO` menu.
...
The wavetable position is controlled by a curve which values changes with
time, with {guilabel}`COLOR` controlling to how the wavetable position goes
back to 0 where the oscillator is a pure sinus, like the 808:
- Counterclockwise, the wavetable oscillator is just a sinus
- Clockwise, the wavetable oscillator reaches the sinus position if the {guilabel}`LENGTH`
  is not too short

Said differently, the {guilabel}`COLOR` paremeter controls the amount and duration
of high frequency transients produced.
...
The wavetable position is controlled by a curve which values changes with
time, with {guilabel}`COLOR` controlling the wavetable position.
...
{guilabel}`COLOR` controls the position of the wavetable. Then a time-varying
